 ...
 * Same shape as /custom-development/articles.
 */
 ...
 * index, scoped to /custom-development/guides. Crossreferences the Organization entity
...
function buildBlogSchema(posts: Post[]) {
  ...
  return {
    ...
    blogPost: recent.map((p) => ({
      ...
      url: `${SITE_URL}/custom-development/guides/${p.slug}`,
        ...
        {posts.length === 0 ? (
          ...
          <div className="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {posts.map((post, index) => (
              <Link
                key={post.slug}
                href={`/custom-development/guides/${post.slug}`}
